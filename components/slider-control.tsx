...
export default function SliderControl({
  ...
  const {
    ...
    handleInputChange,
    handleSliderChange: originalHandleSliderChange,
    resetToDefault,
    showReset,
  } = useSliderWithInput({ minValue, maxValue, initialValue, defaultValue });

  // Wrap slider change handler to call onValueChange
  const handleSliderChange = React.useCallback((newValue: number[]) => {
    originalHandleSliderChange(newValue);
    if (onValueChange) {
      onValueChange(newValue as [number]);
    }
  }, [originalHandleSliderChange, onValueChange]);

  // Wrap validate function to call onValueChange
  const handleValidateAndUpdate = React.useCallback((rawValue: string, index: number) => {
    validateAndUpdateValue(rawValue, index);
    // Call onValueChange after validation with the new value
    if (onValueChange) {
      ...
      if (!isNaN(numValue)) {
        ...
        onValueChange(newSliderValues as [number]);
      }
    }
  }, [validateAndUpdateValue, onValueChange, maxValue, minValue, sliderValue]);
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <Input
            ...
            onBlur={() => handleValidateAndUpdate(inputValues[0] ?? "", 0)}
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                handleValidateAndUpdate(inputValues[0] ?? "", 0);
              }
            }}
